<template>
  <div class="min-h-screen bg-[#313338]">
    <div class="container mx-auto px-4 py-8">
      ...
      <div
        v-if="error"
        class="max-w-md mx-auto mb-8 px-4 sm:px-0"
      >
      ...
      >
        <div
          class="bg-[#2b2d31] rounded-lg p-4 sm:p-6 max-w-md w-full mx-4 border border-[#3f4147]"
        >
      ...
      <div
        v-if="user"
        class="max-w-md mx-auto px-4 sm:px-0"
      >
        <!-- Discord Profile Card -->
        <div
          class="rounded-lg overflow-hidden"
          :style="getCardBackgroundStyle(user)"
        >
          <!-- Banner -->
          <div class="relative h-20 sm:h-24">
            ...
            />
            <div
              v-else-if="user.banner_color"
              class="w-full h-full"
              :style="{ backgroundColor: user.banner_color }"
            />
            <div
              v-else
              class="w-full h-full bg-[#5865f2]"
            />
          </div>
          ...
          <div class="px-3 sm:px-4 pb-4">
            ...
            <div class="relative -mt-6 sm:-mt-8 mb-3">
              <div class="relative inline-block">
                ...
                <!-- Avatar Decoration -->
                <div
                  v-if="user.avatar_decoration_data"
                  class="absolute inset-0 w-12 h-12 sm:w-16 sm:h-16"
                >
                  <img
                    :src="
                      getAvatarDecorationUrl(user.avatar_decoration_data.asset)
                    "
                ...
                <div
                  v-if="user.lanyard"
                  class="absolute bottom-0 right-0 w-3 h-3 sm:w-5 sm:h-5 border-2 border-[#2b2d31] rounded-full"
                  :style="{
            ...
            <!-- Badges -->
            <div
              v-if="getUserBadges(user.public_flags).length > 0"
              class="mb-4"
            >
              <div class="flex flex-wrap gap-1 sm:gap-2">
                <img
                  v-for="badge in getUserBadges(user.public_flags)"
                  :key="badge.name"
                  :src="badge.icon"
                  :alt="badge.name"
                  :title="badge.name"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div class="flex gap-2 mt-1">
                    <span
                      v-if="user.lanyard.active_on_discord_desktop"
                      class="text-[#f2f3f5] text-xs bg-[#5865f2] px-2 py-1 rounded"
                    >Desktop</span>
                    <span
                      v-if="user.lanyard.active_on_discord_web"
                      class="text-[#f2f3f5] text-xs bg-[#5865f2] px-2 py-1 rounded"
                    >Web</span>
                    <span
                      v-if="user.lanyard.active_on_discord_mobile"
                      class="text-[#f2f3f5] text-xs bg-[#5865f2] px-2 py-1 rounded"
                    >Mobile</span>
                  </div>
                </div>
              ...
              <!-- Spotify -->
              <div
                v-if="user.lanyard.listening_to_spotify && user.lanyard.spotify"
                class="rounded p-2 sm:p-3 mb-3"
                :style="getSectionBackgroundStyle(user)"
              >
                <div
                  class="text-[#f2f3f5] text-xs font-medium mb-2 flex items-center gap-2"
                >
                  <svg
                ...
                <div class="flex items-center gap-3">
                  <img
                    v-if="user.lanyard.spotify.album_art_url"
                    :src="user.lanyard.spotify.album_art_url"
                    :alt="user.lanyard.spotify.album"
                    class="w-12 h-12 sm:w-16 sm:h-16 rounded flex-shrink-0"
                  >
                  <div class="min-w-0 flex-1">
                    <div
                      class="text-[#f2f3f5] text-xs sm:text-sm font-medium break-words"
                    >
              ...
              >
                ...
                <div class="space-y-3">
                  <div
                    v-for="activity in user.lanyard.activities.filter(
                      (activity) => activity.name !== 'Spotify',
                    )"
                  ...
                  >
                    <img
                      ...
                      "
                      :alt="activity.name"
                      class="w-10 h-10 sm:w-12 sm:h-12 rounded flex-shrink-0"
                    >
                    <div class="min-w-0 flex-1">
                      <div
                        class="text-[#f2f3f5] text-xs sm:text-sm font-medium break-words"
                      ...
                      <div
                        ...
                        class="text-[#b5bac1] text-xs break-words"
                      >
                        {{ activity.details }}
                      </div>
                      <div
            ...
            >
              ...
              >
                ...
                <div class="flex items-center gap-2">
                  ...
                  <div class="min-w-0 flex-1">
                    <div
                      class="text-[#f2f3f5] text-xs sm:text-sm font-medium break-all"
                    >
              ...
              >
                ...
                <div class="space-y-1 text-xs">
                  <div class="flex justify-between items-start gap-2">
                    ...
                    <span
                      class="text-[#f2f3f5] font-mono break-all text-right"
                    >{{ user.id }}</span>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2 min-w-0">
                      <div
                        class="w-3 h-3 rounded-full border border-[#3f4147] flex-shrink-0"
                        :style="{
                          backgroundColor: `#${user.accent_color.toString(16).padStart(6, '0')}`,
                        }"
                      />
                      <span
                        class="text-[#f2f3f5] font-mono text-right break-all"
                      >#{{
                        user.accent_color
                          .toString(16)
                          .padStart(6, "0")
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2 min-w-0">
                      ...
                      <span
                        class="text-[#f2f3f5] font-mono text-right break-all"
                      >{{ user.banner_color.toUpperCase() }}</span>
                    </div>
                  </div>
                  <div class="flex justify-between items-start gap-2">
            ...
            <!-- Nameplate -->
            <div
              v-if="user.collectibles && user.collectibles.nameplate"
              class="mt-4"
            >
              <div
                class="relative rounded-lg overflow-hidden min-h-[60px] sm:min-h-[80px]"
              >
                <video
                  :src="getNameplateUrl(user.collectibles.nameplate.asset)"
                  autoplay
                  loop
                  muted
                ...
                <div class="relative p-2 sm:p-4 z-10">
                  <div class="flex items-center gap-2 sm:gap-3">
                    <div class="relative flex-shrink-0">
                      ...
                      <div
                        v-if="user.avatar_decoration_data"
                        class="absolute inset-0 w-8 h-8 sm:w-12 sm:h-12"
                      >
                        <img
                          :src="
  ...
  <footer class="mt-8 py-6 text-center border-t border-[#3f4147] bg-[#1e1f22]">
    <div class="text-[#b5bac1] text-sm px-4">
      <p class="mb-2">
        <a
          href="https://github.com/sw3do/discord.id-rewritten"
          target="_blank"
          rel="noopener noreferrer"
          class="text-[#5865f2] hover:text-[#4752c4] transition-colors font-medium"
        >Open source</a>
        • Rebuilt by
        <a
          href="https://github.com/sw3do"
          target="_blank"
          rel="noopener noreferrer"
          class="text-[#5865f2] hover:text-[#4752c4] transition-colors font-medium"
        >sw3do</a>
